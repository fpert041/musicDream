...
ofSoundBufferPF::ofSoundBufferPF() { 
	numFrames = 0; 
	numChannels = 0; 
	buffer = NULL; 
}
ofSoundBufferPF::ofSoundBufferPF( const ofSoundBufferPF& other ) { 
	copyFrom( other ); 
}
ofSoundBufferPF::ofSoundBufferPF( int nFrames, int nChannels ) { 
	numFrames = nFrames; 
	numChannels = nChannels; 
	buffer = new float[nFrames*nChannels]; 
}
ofSoundBufferPF& ofSoundBufferPF::operator=( const ofSoundBufferPF& other ) { 
	copyFrom( other ); 
	return *this; 
}
...
void ofSoundBufferPF::copyFrom( const ofSoundBufferPF& other ) { 
...
void ofSoundBufferPF::allocate( int nFrames, int nChannels ) { 
...
void ofSoundBufferPF::copyChannel( int channel, float* output ) const {
...
void ofSoundBufferPF::copyTo( float* outBuffer, int outNumFrames, int outNumChannels ) {
	if ( numFrames == 0 || numChannels == 0 ) {
		ofLog( OF_LOG_WARNING, "ofSoundBufferPF::copyTo: copy requested on empty buffer, returning nothing (check your addInputTo() methods!)" );
		memset( outBuffer, 0, sizeof(float)*outNumFrames*outNumChannels );
	}
	if ( outNumFrames>numFrames ||  outNumChannels>numChannels ) {
		ofLog( OF_LOG_WARNING, "ofSoundBufferPF::copyTo: %i frames requested but only %i	are available", outNumFrames, numFrames );
		ofLog( OF_LOG_WARNING, "ofSoundBufferPF::copyTo: %i channels requested but only %i are available, looping to make up the difference", outNumChannels, numChannels );
